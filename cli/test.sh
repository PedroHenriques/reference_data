...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --coverage) COVERAGE="/p:CollectCoverage=true /p:CoverletOutput=./test/coverage/ /p:CoverletOutputFormat=opencover /p:Exclude=\"[xunit.*]*\""; FILTERS="--filter Type=Unit"; TEST_TYPE="unit"; shift 1;;
